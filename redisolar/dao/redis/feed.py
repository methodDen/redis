...
class FeedDaoRedis(FeedDaoBase, RedisDaoBase):
    ...
    def _insert(self, meter_reading: MeterReading,
        ...
        meter_reading_data = MeterReadingSchema().dump(meter_reading)
        pipeline.xadd(global_key, meter_reading_data, maxlen=self.GLOBAL_MAX_FEED_LENGTH)
        pipeline.xadd(site_key, meter_reading_data, maxlen=self.SITE_MAX_FEED_LENGTH)
